...
The Windows supervisor starts the virtual environment's base Python directly
with CPython's `__PYVENV_LAUNCHER__` handoff set to the managed venv executable.
This retains the venv's interpreter identity and installed packages while making
the supervised PID the actual model worker. Queue recovery, emergency Stop and
worker replacement must all refer to that same PID; launching the Windows venv
redirector instead introduces a child process and breaks that ownership check.
This variable is set only in the supervised child's environment, not as an
operator setting. Other platforms and non-venv launches are unchanged. See
[CPython's executable and virtual environment discovery](https://github.com/python/cpython/blob/3.12/Modules/getpath.py).

Worker and supervisor queue publication share a bounded Windows retry for atomic
replacement: at most five attempts with four 10 ms delays for access/sharing
violations. This lets a concurrent snapshot reader finish without dropping the
pending progress or terminal update. The previous file stays intact until its
replacement succeeds; permanent errors remain visible. Other platforms and
unrelated I/O errors are not retried. See
[Windows file-sharing modes](https://learn.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-createfilew).
