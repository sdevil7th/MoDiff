...
| P6.81 Bounded DreamLite app download selections | `bcad80a` | Not required | Exact immutable app plans only; remote real-weight execution/output review and physical macOS remain pending | Not required | One shared exact 27-file component inventory seals the Base and Mobile snapshots at 10,143,788,478 logical bytes, including the separately consumed processor and tokenizer trees and the CC-BY-NC model card, with safetensors-only weights. Base is already complete. Mobile still needs 5,071,894,300 bytes, but its fresh aggregate plan was short by 7,735,141,399 bytes, so no POST, interruption, or deletion occurred. |
