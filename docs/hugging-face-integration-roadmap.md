...
- [ ] **P0.3 Canonical execution-spec registry and generic node closure**
  ...
  - [ ] **P0.3b Standard adapter fail-closed closure**
    ...
    - [x] **P0.3b.6 Generic auxiliary LoRA identity:** replace mutable and
      ...
      assets: none.
    - [ ] **P0.3b.7 Paired checkpoint:** run focused and complete backend/client
    ...
    - Checkpoint limitation 2026-08-09: the reviewed client bundle was mirrored
      byte-for-byte (`index.js` SHA-256
      `c47e3a08fb75d1427d0104e3cfe0f16a3331382357efbaf4b4ec67f323902be9`,
      1,301,204 bytes), all 317 backend-owned local Gallery files remained
      present, and fresh HTTP requests returned 200 for the app, bundle,
      health/runtime status, and a preserved Gallery asset. P0.3b.7 and the
      parent remain open because paired commit references are still pending and
      the required `npm run check:ui` has no checked-in Windows visual
      baselines. The shared behavior assertion and
      focused browser contracts pass, but generated Win32 PNG/JSON baselines were
      removed rather than accepted without review. No model, media, Gallery
      asset, Transformers installation, network model download, or GPU execution
      occurred. The separate P0.5 clean-base migration remains required before
      the base installer can claim that Transformers and PEFT are absent by
      default.
