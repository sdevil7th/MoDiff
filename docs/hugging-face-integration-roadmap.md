...
| P6.93 Bounded admitted auxiliary downloads | `cc748d0` | Not required | Exact immutable app plans plus prior source/contract qualification only; current-worker activation, remote real-weight execution/output review, and physical macOS remain pending | Not required | Exact 4/4/4/4/4/4/5-file selections seal 13,476,063,409 logical bytes across Qwen Image ControlNet Union, Hunyuan-DiT v1.2 Canny ControlNet, SDXL T2I-Adapter Canny, SDXL ControlNet Canny, AnimateDiff MotionAdapter, SD1.5 ControlNet Canny, and AnimateLCM MotionAdapter plus LoRA. The selections exclude repository Python, demo media, legacy `.bin`/`.ckpt` weights, and unused full-precision duplicates while retaining the precise safe variant each existing loader consumes. Every admitted dependency repository now resolves a non-empty app selection; the FLUX Redux base requirement reuses its already-bounded primary FLUX.1-dev selection. Fresh exact app plans on 2026-08-14 report Qwen Image ControlNet Union's 3,536,036,150 bytes, Hunyuan Canny's 2,976,971,742 bytes, SDXL T2I-Adapter Canny's 158,071,198 bytes, SD1.5 ControlNet Canny's 1,445,176,301 bytes, AnimateDiff's 1,815,334,485 bytes, and AnimateLCM's 1,042,327,684 bytes complete at their immutable revisions. All six cache entries are installed, complete, and repair-free with no active, missing, or corrupt files. SDXL ControlNet Canny's fp16 weight and config are also installed, complete, and repair-free: 2,502,140,445 of its 2,502,145,849 exact bytes are present, leaving only 5,404 model-card/attribute receipt bytes. A dedicated app-only repair monitor performs two fresh queue-aware plans before any future POST and currently holds because the active queue plus reserve does not fit. The live worker predates the bounded-selection source, so automatic selection is still source/test evidence for the later safe restart rather than a claim about its current request shape. The affected 277-test matrix passes with 645 subtests and five expected skips; the complete backend gate passes 1,690 tests, 3,438 subtests, and 40 expected skips together with Ruff, 66-package compatibility, shell, preflight, and diff checks. No duplicate POST, interruption, deletion, generation, output, or physical macOS qualification occurred. |
...
| P6.98 Live bounded-transfer cutover and pinned Gallery manifest dimensions | `578e0a3` | Not required | Current Linux worker activation, focused source/unit tests, immutable manifest validation, and read-only live app plans only; Gallery installation, remote generation, and physical macOS remain pending | Not required | After the preserved old app queue drained, the supervisor was restarted onto the current source. The live schema-v1 download status now reports zero active transfers and reservations, the runtime endpoint is ready, and future model admissions use the two-transfer bounded implementation. The pinned Gallery manifest contains the original five-field shape for 186 records and an exact `width`/`height` pair for 170 image records; the backend now accepts only that bounded positive image-dimension extension while rejecting partial pairs, non-image dimensions, out-of-range values, and arbitrary metadata. The immutable 356-file / 480,430,370-byte manifest validates at asset-set identity `sha256:canonical-json:5ec869b755a6ce04a789d6835819da150493bfaef8a6bc1480f0274ba05bcab9`, and 12 focused Gallery tests pass. The activated app plan reserves 969,249,348 bytes for download plus staging but reports only 66,333,192,192 free bytes against the 68,719,476,736-byte safety reserve, so it correctly refuses installation. A dedicated app-only monitor performs two fresh plans before any future Gallery POST. No model or asset byte was deleted or fetched outside the app, and no graph, inference, output, or physical macOS claim occurred. |
