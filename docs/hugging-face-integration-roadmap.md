...
- [ ] **P0.3 Canonical execution-spec registry and generic node closure**
  ...
  - [ ] **P0.3e Remove remaining frontend and node model switches**
    ...
    - [x] `FluxCannyPipeline:control_image`: backend commit `14fef9f` moves its
      ...
      model execution, generated media, or live workload qualification occurred.
    - [ ] Continue one exact model/mode pair per paired commit; Krea, Depth, and
      Canny completion does not qualify the remaining Kontext, Fill, Redux,
...
| P0.3e | `96f70cb` (Flux Krea T2I), `a299d1d` (Flux Depth control-image), `14fef9f` (Flux Canny control-image); add one row/reference per remaining pair | `80ac243` (Flux Krea T2I), `2de0c68` (Flux Depth control-image), `784e3c7` (Flux Canny control-image); add one row/reference per remaining pair | Not required | Not required | In progress: the exact Flux Krea text-to-image and Flux Depth/Canny control-image pairs are backend-specification-owned and passed complete backend/client/mocked-browser gates. Remaining exact pairs and declarative overlays are pending. |
