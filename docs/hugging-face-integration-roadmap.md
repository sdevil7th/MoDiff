...
- [ ] **P2.5 Remote Gallery qualification and activation**
  ...
  - [x] **P2.5g Live download-idle and Gallery-plan cutover:** backend
    ...
    files / 33,867,388 bytes. The app's Gallery plan initially refused its
    ...
    and verified all 356 assets / 480,430,370 bytes at the pinned revision. A
    safe worker restart is still required to register the new static route and
    is deferred while bounded model downloads are active. No model cache entry
  ...
  - [x] **P2.5h Current-attempt transfer progress reservations:** backend
    ...
    focused download/Gallery matrix passes 65 tests and 70 subtests, with Python
    compilation and diff checks clean. The live worker intentionally predates
    this change until its current queue drains. No transfer was interrupted, no
    cache entry was deleted, and no graph, inference, output, or physical macOS
    evidence is claimed.
...
| P2.5g Live download-idle and Gallery-plan cutover | `578e0a3` | `3413455` | Current Linux worker readiness, immutable app install, and generation-free dry-run evidence only; post-install restart, 76 live output receipts, review/publication, activation, and physical macOS remain pending | Pending | The restarted current worker exposes the bounded idle-status contract. The immutable 356-file / 480,430,370-byte Gallery manifest validates its bounded image dimensions and exact asset-set identity. Its first app plan correctly refused a 969,249,348-byte download/staging reservation while free space was below the 68,719,476,736-byte reserve. After 62.8 GiB of disposable non-model uv cache was cleared, two fresh app plans fit and the app downloaded, hashed, atomically promoted, and verified the complete Gallery payload. The dry run before installation reported 76/76 jobs and 31/31 artifacts app-ready, download readiness ready, and 38/76 jobs blocked on 50 absent inputs / 33,867,388 bytes. A safe restart and new input-readiness run remain deferred while bounded model transfers are active. No model deletion, graph, inference, or output occurred. |
...
| P6.98 Live bounded-transfer cutover and pinned Gallery manifest dimensions | `578e0a3` | Not required | Current Linux worker activation, focused source/unit tests, immutable manifest validation, and live app install evidence only; post-install restart, remote generation, and physical macOS remain pending | Not required | After the preserved old app queue drained, the supervisor was restarted onto the current source. The live schema-v1 download status and runtime endpoint are active, and new admissions now use the two-transfer bounded implementation. The pinned Gallery manifest contains the original five-field shape for 186 records and an exact `width`/`height` pair for 170 image records; the backend accepts only that bounded positive image-dimension extension while rejecting partial pairs, non-image dimensions, out-of-range values, and arbitrary metadata. The immutable 356-file / 480,430,370-byte manifest validates at asset-set identity `sha256:canonical-json:5ec869b755a6ce04a789d6835819da150493bfaef8a6bc1480f0274ba05bcab9`, and 12 focused Gallery tests pass. After disposable non-model uv cache cleanup, two fresh app plans admitted the exact 969,249,348-byte download/staging reservation and the app installed and verified all 480,430,370 final bytes. SDXL Turbo and CogVideoX now download concurrently while SANA Video waits in the app queue, proving the bounded cutover is live. A safe Gallery-route restart remains deferred until transfers are idle. No model or asset byte was deleted or fetched outside the app, and no graph, inference, output, or physical macOS claim occurred. |
| P6.99 Current-attempt transfer progress accounting | `c140b3f` | Not required | Focused source/unit proof only; current-worker activation waits for the active app queue to drain, while generation and physical macOS remain pending | Not required | Resumed downloads no longer count abandoned pre-task partials or duplicate logical cache links as current progress. Immutable LFS hash prefixes are matched across standard Hub per-attempt suffixes; only files touched by the current task shrink its reservation, while retry files remain conservatively accounted as distinct disk consumption and unhashed metadata remains reserved until complete. The 65-test / 70-subtest focused download/Gallery matrix, Python compilation, and diff checks pass. The live worker predates this source correction and remains uninterrupted; no cache entry was deleted and no graph or inference ran. |
