...
class AcceleratorManifestTests(unittest.TestCase):
    def test_all_public_profiles_are_release_pinned(self):
        ...
        self.assertEqual(set(manifest["profiles"]), {"nvidia-cuda", "amd-rocm-linux", "amd-pytorch-windows", "apple-mps", "cpu"})
        for name, profile in manifest["profiles"].items():
            with self.subTest(name=name):
                ...
                self.assertIn(profile["tier"], {"supported", "preview", "conditional"})
