...
from modules.DiffusersThreeD.main import (
    ...
    SHAP_E_IMG2IMG_ADAPTER,
    SHAP_E_IMG2IMG_MODE,
    SHAP_E_IMG2IMG_PIPELINE_CLASS,
    SHAP_E_IMG2IMG_REPO,
    SHAP_E_IMG2IMG_REVISION,
...
class FakeShapEImg2ImgPipeline(FakeShapEPipeline):
    _modiff_three_d_pipeline_class = SHAP_E_IMG2IMG_PIPELINE_CLASS
    _modiff_three_d_mode = SHAP_E_IMG2IMG_MODE
    _modiff_three_d_repo = SHAP_E_IMG2IMG_REPO
    _modiff_three_d_revision = SHAP_E_IMG2IMG_REVISION
...
class DiffusersThreeDTests(unittest.TestCase):
    ...
    def test_exact_artifact_and_contract_are_fail_closed(self):
        ...
        self.assertEqual(
            _require_exact_selection(
                {"source": "hub", "value": SHAP_E_IMG2IMG_REPO},
                SHAP_E_IMG2IMG_REVISION,
                adapter=SHAP_E_IMG2IMG_ADAPTER,
            ),
            (SHAP_E_IMG2IMG_REPO, SHAP_E_IMG2IMG_REVISION),
        )
        with self.assertRaises(ValueError):
            _require_exact_selection(
                {"source": "hub", "value": SHAP_E_REPO},
    ...
    @patch("diffusers.PriorTransformer.from_pretrained")
    @requires_transformers
    def test_image_conditioned_assembly_uses_only_reviewed_safe_components(
        self,
        prior_loader,
        vision_loader,
        processor_loader,
        ...
        self.assertEqual(
            set(components),
            {"prior", "image_encoder", "image_processor", "scheduler", "shap_e_renderer"},
        )
        for loader in (prior_loader, vision_loader, renderer_loader):
            kwargs = loader.call_args.kwargs
            self.assertTrue(kwargs["use_safetensors"])
            self.assertEqual(kwargs["variant"], "fp16")
            self.assertEqual(kwargs["revision"], SHAP_E_IMG2IMG_REVISION)
        self.assertEqual(vision_loader.call_args.kwargs["subfolder"], "image_encoder")
        self.assertEqual(processor_loader.call_args.kwargs["subfolder"], "image_processor")
        self.assertNotIn("use_safetensors", processor_loader.call_args.kwargs)
        self.assertEqual(renderer_loader.call_args.kwargs["subfolder"], SHAP_E_RENDERER_SAFE_SUBFOLDER)
        self.assertEqual(scheduler_loader.call_args.kwargs["subfolder"], "scheduler")
    ...
    def test_loader_assembles_image_conditioned_pipeline_with_exact_identity(self):
        components = {"component": object()}
        node = LoadPipeline("three-d-image-loader")
        node.progress = Mock()
        node.mm_add = Mock()
        node.diffusers_loading_progress = Mock(return_value=nullcontext())
        with (
            patch("diffusers.ShapEImg2ImgPipeline", FakeShapEImg2ImgPipeline),
            patch("modules.DiffusersThreeD.main._load_safe_shap_e_components", return_value=components) as loader,
            patch(
                "modules.DiffusersRuntime.main.loader_runtime_options",
                return_value=({}, "cpu", "none", {}),
            ),
            patch("modules.DiffusersThreeD.main.apply_pipeline_offload"),
        ):
            result = node.execute(
                model_id={"source": "hub", "value": SHAP_E_IMG2IMG_REPO},
        ...
        self.assertEqual(result["resolved_artifact"], SHAP_E_IMG2IMG_REPO)
        self.assertEqual(result["pipeline"]._modiff_three_d_mode, SHAP_E_IMG2IMG_MODE)
        self.assertEqual(loader.call_args.kwargs["adapter"], SHAP_E_IMG2IMG_ADAPTER)
    ...
    def test_image_conditioned_generation_forwards_one_bounded_image(self):
        pipeline = FakeShapEImg2ImgPipeline()
        reference = Image.new("RGB", (512, 384), "blue")
        result = GenerateRenderedArtifact("three-d-image-generate").execute(
            pipeline=pipeline,
            three_d_contract=SHAP_E_IMG2IMG_ADAPTER.signal_value(),
            prompt="",
        ...
        self.assertEqual(result["video"], pipeline.frames)
        call = {key: value for key, value in pipeline.call_kwargs.items() if key != "generator"}
        self.assertIs(call.pop("image"), reference)
        self.assertEqual(
            call,
            {
                "num_images_per_prompt": 1,
        ...
        for invalid, message in (
            ([], "exactly one"),
            ([reference, reference], "exactly one"),
            ([Image.new("RGB", (2049, 2049))], "4,194,304"),
        ):
            with self.subTest(invalid=len(invalid)), self.assertRaisesRegex(ValueError, message):
                GenerateRenderedArtifact().execute(
                    pipeline=pipeline,
                    three_d_contract=SHAP_E_IMG2IMG_ADAPTER.signal_value(),
                    reference_images=invalid,
                )
